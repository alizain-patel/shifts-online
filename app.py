...
# ---------------------------------------------------------------
# DATETIME PARSING (PER-ROW, TZ-AWARE)
# ---------------------------------------------------------------
# We found raw JSON often contains strings like: Timestamp('YYYY-MM-DD HH:MM:SS')
# The helpers below robustly clean & parse those to build a single tz-aware IST datetime.

_TS_PATTERN = r"^Timestamp\('(.*?)'\)$"

def _clean_ts_string(s: pd.Series) -> pd.Series:
    """
    Clean strings of the form: Timestamp('YYYY-MM-DD HH:MM:SS')
    to: 'YYYY-MM-DD HH:MM:SS'. Non-matching values are unchanged.
    """
    if s.dtype == 'O':
...
def _to_dt_localize_ist(ser: pd.Series) -> pd.Series:
    """
    Parse naive strings to datetime and localize to IST.
    If the parsed datetime is already tz-aware, convert to IST.
    """
    ser = _clean_ts_string(ser)
    dt  = pd.to_datetime(ser, errors='coerce')
    # If tz-aware -> convert; else -> localize
    return (dt.dt.tz_convert(IST_TZ) if getattr(dt.dt, 'tz', None) is not None
...
def build_datetime_rowwise(df: pd.DataFrame) -> pd.Series:
    """
    Build a single tz-aware IST datetime **per row** with this priority:
      ...
      3) legacy 'datetime' — intended IST -> localize IST
         (Handles strings like Timestamp('YYYY-MM-DD ...'))

      4) 'date' + 'time' — localize IST
         (Also handles Timestamp('...') source)

    This order ensures **newer ISO rows win**, while legacy values remain a fallback.
    """
    n = len(df)
    ...
    if 'sort_key' in df.columns:
        ...
        if tzcol is not None:
            ...
            if ist_mask.any():
                dt_sort.loc[ist_mask] = _to_dt_localize_ist(naive.loc[ist_mask])
            if utc_mask.any():
                dt_sort.loc[utc_mask] = _to_dt_utc_to_ist(naive.loc[utc_mask])
        else:
    ...
    if 'datetime_iso' in df.columns:
        ...
        if tzcol is not None:
            ...
            utc_mask = (~ist_mask) & base.notna()
            if ist_mask.any():
                dt_iso.loc[ist_mask] = _to_dt_localize_ist(naive.loc[ist_mask])
            if utc_mask.any():
                dt_iso.loc[utc_mask] = _to_dt_utc_to_ist(naive.loc[utc_mask])
        else:
    ...
    # Coalesce in this order: ISO first → legacy → date+time
    return dt_sort.combine_first(dt_iso).combine_first(dt_legacy).combine_first(dt_dt)
...
# Path + mtime (helps you confirm the exact file the app is reading)
try:
...
# Friday→Today (Mon: Fri→Mon) window
...
if view_mode == "Latest per user" and "user_id" in df.columns:
    if prefer_today:
        ...
        if not df_today.empty:
            latest_today = (df_today.sort_values("datetime", ascending=False)
                                    .drop_duplicates(subset=["user_id"], keep="first"))
            missing_users = set(df["user_id"]) - set(latest_today["user_id"])
            latest_other  = (df_other[df_other["user_id"].isin(missing_users)]
                                    .sort_values("datetime", ascending=False)
                                    .drop_duplicates(subset=["user_id"], keep="first"))
            df = pd.concat([latest_today, latest_other], ignore_index=True).sort_values("datetime", ascending=False)
        else:
            df = (df.sort_values("datetime", ascending=False)
                    .drop_duplicates(subset=["user_id"], keep="first")
                    .sort_values("datetime", ascending=False))
